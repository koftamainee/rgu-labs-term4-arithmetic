...
public:
  Polynomial1D(const VectorBF &coeffs, const bigfloat &a = bigfloat(0))
      : coeffs_(coeffs), a_(a) {}
  ...
  Polynomial1D change_expansion_point(const bigfloat &B) const {
    if (B == a_) {
      return *this;
    }
    ...
    return Polynomial1D(new_coeffs, B);
  }
  ...
  Polynomial1D operator+(const Polynomial1D& other) const {
    ...
    return Polynomial1D(VectorBF(result));
  }
  ...
  Polynomial1D operator*(const bigfloat& scalar) const {
  ...
  Polynomial1D operator*(const Polynomial1D& other) const {
    ...
    if (fn == 0 || gn == 0)
      return Polynomial1D(VectorBF(std::vector<bigfloat>{bigfloat(0)}));
  ...
  Polynomial1D rem_xn_minus_1(const size_t n) const {
    std::vector<bigfloat> result(n, bigfloat(0));
    for (size_t i = 0; i < coeffs_.dimension(); ++i) result[i % n] += coeffs_[i];
    return Polynomial1D(VectorBF(result));
  }

  Polynomial1D rem_xn_plus_1(const size_t n) const {
    std::vector<bigfloat> result(n, bigfloat(0));
  ...
  Polynomial1D substitute_wx(const std::vector<bigfloat>& omega_powers) const {
  ...
  Polynomial1D first_n_coeffs(const size_t n) const {
    std::vector<bigfloat> result(n, bigfloat(0));
