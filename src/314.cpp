...
int main() {
    ...
    std::cout << "    Convergence for ANY x0 iff |a - b| < 1  (c is free)\n\n";

    struct CaseA { double a, b, c; std::vector<double> x0s; };
    ...
    std::cout << "    Convergence for ANY x0 iff b > 0  AND  2*a^2*b < e = "
              << std::fixed << std::setprecision(10) << std::exp(1.0) << "\n\n";

    struct CaseB { double a, b, c; std::vector<double> x0s; };
